################################################################################
# Phospho GSEA without PN Meta vs Phospho GSEA with PN Meta Integration
#
# Purpose:
#   Integrate phosphoprotein-level GSEA meta-analysis results from
#   without-protein-normalization and with-protein-normalization pipelines
#   by merging on the "pathway" column for each collection and comparison pair.
#
# Input:
#   - phosphoprotein_GSEA_without_PN_meta/{collection}/META_GSEA_{comparison}.csv
#     (from phosphoprotein_without_PN_GSEA_subgroup_adjusted_meta_analysis.R)
#   - phosphoprotein_GSEA_with_PN_meta/{collection}/META_GSEA_{comparison}.csv
...
# Input directories
without_PN_meta_dir <- file.path(base_path, "phosphoprotein_GSEA_without_PN_meta")
...
# PTMsigDB collection (phosphoprotein level)
collection_names <- c(
    "PTMsigDB"
)

cat("====================================================================\n")
cat("Phospho GSEA without PN Meta vs Phospho GSEA with PN Meta Integration\n")
cat("====================================================================\n\n")
    ...
    for (j in seq_len(nrow(comparisons))) {
        ...
        if (!file.exists(without_PN_file) && !file.exists(with_PN_file)) {
            ...
            integration_summary[[paste(coll_name, comp_name, sep = "_")]] <- data.frame(
                ...
                n_phospho_GSEA_without_PN_pathways = NA,
                n_phospho_GSEA_with_PN_pathways = NA,
        ...
        # --- Load phospho_GSEA_without_PN meta-GSEA data ---
        ...
        if (file.exists(without_PN_file)) {
            ...
            if (nrow(tmp_without_PN) > 0 && "pathway" %in% colnames(tmp_without_PN)) {
                # Rename all columns except "pathway" with _phospho_GSEA_without_PN suffix
                cols_to_rename <- colnames(tmp_without_PN)[colnames(tmp_without_PN) != "pathway"]
                setnames(tmp_without_PN, cols_to_rename, paste0(cols_to_rename, "_phospho_GSEA_without_PN"))
        ...
        # --- Load phospho_GSEA_with_PN meta-GSEA data ---
        ...
        if (file.exists(with_PN_file)) {
            ...
            if (nrow(tmp_with_PN) > 0 && "pathway" %in% colnames(tmp_with_PN)) {
                # Rename all columns except "pathway" with _phospho_GSEA_with_PN suffix
                cols_to_rename <- colnames(tmp_with_PN)[colnames(tmp_with_PN) != "pathway"]
                setnames(tmp_with_PN, cols_to_rename, paste0(cols_to_rename, "_phospho_GSEA_with_PN"))
                # Remove duplicate pathways
        ...
        if (nrow(without_PN_data) == 0 && nrow(with_PN_data) == 0) {
            ...
            integration_summary[[paste(coll_name, comp_name, sep = "_")]] <- data.frame(
                collection = coll_name,
                comparison = comp_name,
                n_phospho_GSEA_without_PN_pathways = 0,
                n_phospho_GSEA_with_PN_pathways = 0,
                n_merged_pathways = 0,
                n_both_sig_005 = NA,
                stringsAsFactors = FALSE
            )
        ...
        # Identify per-cancer NES and pval columns for phospho_GSEA_without_PN and phospho_GSEA_with_PN
        all_cols <- colnames(merged)

        nes_without_PN_cols  <- grep("^NES_.*_phospho_GSEA_without_PN$",  all_cols, value = TRUE)
        pval_without_PN_cols <- grep("^pval_.*_phospho_GSEA_without_PN$", all_cols, value = TRUE)
        nes_with_PN_cols     <- grep("^NES_.*_phospho_GSEA_with_PN$",  all_cols, value = TRUE)
        pval_with_PN_cols    <- grep("^pval_.*_phospho_GSEA_with_PN$", all_cols, value = TRUE)

        # Extract cancer type names from NES columns to ensure matched pairing
        without_PN_cancers <- sub("^NES_(.*)_phospho_GSEA_without_PN$", "\\1", nes_without_PN_cols)
        with_PN_cancers    <- sub("^NES_(.*)_phospho_GSEA_with_PN$", "\\1", nes_with_PN_cols)

        # phospho_GSEA_without_PN: count datasets with significant up (NES > 0 & pval < 0.05) and down
        merged$without_PN_up_datasets_num   <- 0L
        merged$without_PN_down_datasets_num <- 0L
        for (ct in without_PN_cancers) {
            nes_col  <- paste0("NES_", ct, "_phospho_GSEA_without_PN")
            pval_col <- paste0("pval_", ct, "_phospho_GSEA_without_PN")
        ...
        # phospho_GSEA_with_PN: count datasets with significant up and down
        merged$with_PN_up_datasets_num   <- 0L
        merged$with_PN_down_datasets_num <- 0L
        for (ct in with_PN_cancers) {
            nes_col  <- paste0("NES_", ct, "_phospho_GSEA_with_PN")
            pval_col <- paste0("pval_", ct, "_phospho_GSEA_with_PN")
            if (nes_col %in% all_cols && pval_col %in% all_cols) {
                nes_vals  <- merged[[nes_col]]
                pval_vals <- merged[[pval_col]]
                is_up   <- !is.na(nes_vals) & !is.na(pval_vals) & nes_vals > 0 & pval_vals < 0.05
        ...
        # phospho_GSEA_without_PN: names of significantly up/down datasets
        ...
        for (i in seq_len(nrow(merged))) {
            ...
            for (ct in without_PN_cancers) {
                nes_val  <- merged[[paste0("NES_", ct, "_phospho_GSEA_without_PN")]][i]
                pval_val <- merged[[paste0("pval_", ct, "_phospho_GSEA_without_PN")]][i]
        ...
        # phospho_GSEA_with_PN: names of significantly up/down datasets
        ...
        for (i in seq_len(nrow(merged))) {
            ...
            down_names <- character()
            for (ct in with_PN_cancers) {
                nes_val  <- merged[[paste0("NES_", ct, "_phospho_GSEA_with_PN")]][i]
                pval_val <- merged[[paste0("pval_", ct, "_phospho_GSEA_with_PN")]][i]
                if (!is.na(nes_val) && !is.na(pval_val) && pval_val < 0.05) {
                    if (nes_val > 0) up_names   <- c(up_names, ct)
                    if (nes_val < 0) down_names <- c(down_names, ct)
                }
        ...
        # Compute n_both_sig (both phospho_GSEA_without_PN and phospho_GSEA_with_PN padj < 0.05)
        n_both_sig <- NA
        if ("padj_phospho_GSEA_without_PN" %in% colnames(merged) && "padj_phospho_GSEA_with_PN" %in% colnames(merged)) {
            n_both_sig <- sum(
                merged$padj_phospho_GSEA_without_PN < 0.05 & merged$padj_phospho_GSEA_with_PN < 0.05,
                na.rm = TRUE
            )
        }
        ...
        cat("    Phospho GSEA without PN pathways:", n_without_PN, "\n")
        cat("    Phospho GSEA with PN pathways:", n_with_PN, "\n")
        ...
        has_both <- "Z_meta_phospho_GSEA_without_PN" %in% colnames(merged) &
                    "Z_meta_phospho_GSEA_with_PN" %in% colnames(merged) &
                    "padj_phospho_GSEA_without_PN" %in% colnames(merged) &
                    "padj_phospho_GSEA_with_PN" %in% colnames(merged)

        if (has_both) {
            z_wo  <- merged$Z_meta_phospho_GSEA_without_PN
            z_w   <- merged$Z_meta_phospho_GSEA_with_PN
            pa_wo <- merged$padj_phospho_GSEA_without_PN
            pa_w  <- merged$padj_phospho_GSEA_with_PN
        ...
        integration_summary[[paste(coll_name, comp_name, sep = "_")]] <- data.frame(
            collection = coll_name,
            comparison = comp_name,
            n_phospho_GSEA_without_PN_pathways = n_without_PN,
            n_phospho_GSEA_with_PN_pathways = n_with_PN,
...
# Build pathway -> gene set size lookup from per-cancer GSEA input files
# Since PTMsigDB is not in msigdbr, extract sizes from individual GSEA results
cat("Building PTMsigDB gene set size lookup from per-cancer GSEA files...\n")

cancer_types <- c("BRCA", "CCRCC", "COAD", "GBM", "HNSCC", "LSCC", "LUAD", "OV", "PDAC", "UCEC")
gsea_without_PN_input_dir <- file.path(base_path, "phosphoprotein_GSEA_without_protein_normalization_subgroup_adjusted")
gsea_with_PN_input_dir    <- file.path(base_path, "phosphoprotein_GSEA_subgroup_adjusted")

pathway_sizes_list <- list()
for (ct in cancer_types) {
    for (input_dir in c(gsea_without_PN_input_dir, gsea_with_PN_input_dir)) {
        for (coll in collection_names) {
            # Read any one comparison file to get sizes
            gsea_file <- file.path(input_dir, ct, coll, "GSEA_TP53mt_vs_TP53wt.csv")
            if (file.exists(gsea_file)) {
                tmp <- tryCatch(fread(gsea_file, select = c("pathway", "size")), error = function(e) NULL)
                if (!is.null(tmp) && nrow(tmp) > 0 && all(c("pathway", "size") %in% colnames(tmp))) {
                    pathway_sizes_list[[length(pathway_sizes_list) + 1]] <- tmp
                }
            }
        }
    }
...
if (length(pathway_sizes_list) > 0) {
    all_sizes <- rbindlist(pathway_sizes_list)
    # Take median size per pathway across all cancer types
    pathway_size_df <- all_sizes[, .(gene_set_size = as.integer(median(size, na.rm = TRUE))), by = pathway]
    pathway_size_lookup <- setNames(pathway_size_df$gene_set_size, pathway_size_df$pathway)
    cat("  Loaded sizes for", length(pathway_size_lookup), "gene sets\n\n")
} else {
    pathway_size_lookup <- c()
    cat("  WARNING: No gene set sizes found. Points will use uniform size.\n\n")
}

# Plot PTMsigDB collection
collections_to_plot <- c("PTMsigDB")
...
for (coll_name in collections_to_plot) {
    ...
    for (j in seq_len(nrow(comparisons))) {
        ...
        df <- df[is.finite(Z_meta_phospho_GSEA_without_PN) & is.finite(Z_meta_phospho_GSEA_with_PN)]
        ...
        df$fill_color[df$padj_phospho_GSEA_without_PN > 0.05 & df$padj_phospho_GSEA_with_PN > 0.05] <- "light_grey" # Rule 2

        # Identify highlighted pathways (Rule 1)
        # Red: Z_meta_without_PN > 0 & Z_meta_with_PN > 0 & both padj < 0.05
        path_red <- df$pathway[df$Z_meta_phospho_GSEA_without_PN > 0 & df$Z_meta_phospho_GSEA_with_PN > 0 & df$padj_phospho_GSEA_without_PN < 0.05 & df$padj_phospho_GSEA_with_PN < 0.05]
        ...
        df$Z_sum <- df$Z_meta_phospho_GSEA_without_PN + df$Z_meta_phospho_GSEA_with_PN
        ...
        # Format labels (PTMsigDB pathways: keep as-is)
        df$label <- df$pathway
