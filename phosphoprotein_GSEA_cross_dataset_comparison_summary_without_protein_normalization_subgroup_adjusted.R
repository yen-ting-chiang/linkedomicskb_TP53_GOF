...
#
# Outputs: Cross_Dataset_Comparison_Summary_PTMsigDB.csv/xlsx in phosphoprotein_GSEA_without_protein_normalization_subgroup_adjusted/
################################################################################
            ...
            if (file.exists(gsea_file)) {
                ...
                if (!is.null(res) && nrow(res) > 0 && "pathway" %in% colnames(res)) {
                    ...
                    sig_res <- res %>%
                        ...
                        dplyr::select(pathway, NES, dataset)
