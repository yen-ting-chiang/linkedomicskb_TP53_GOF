################################################################################
# TP53 Phosphoprotein Meta-Analysis Gene-Level Preranked GSEA
# (without protein normalization)
#
# Input: Meta-analysis results (META_DPS_*.csv) from
#   phosphoprotein_DPS_without_protein_normalization_meta_analysis/
#   These files contain Stouffer's weighted Z_meta scores per phosphosite.
#
# Two gene-level aggregation methods:
#   1. Max Abs: gene score = Z_meta of the phosphosite with the largest
#      |Z_meta| per gene (direction preserved)
#      Reference: Krug et al. Cell 2020 (CPTAC); Mertins et al. Nature 2016
#   2. Median: gene score = median Z_meta across all phosphosites per gene
...
#   This corresponds to the MAPE_G approach (Shen & Tseng, Bioinformatics
#   2010; PMID: 20410053), where gene-level meta-statistics are first
#   combined across studies, and the resulting meta-statistics are used as
#   ranking for preranked GSEA.
#
# Output:
#   phosphoprotein_DPS_without_protein_normalization_meta_analysis/
#     phosphoprotein_meta_gene_level_GSEA/
#       max_abs_z/  : Gene score = Z_meta of strongest phosphosite per gene
#       median_z/   : Gene score = median Z_meta across all phosphosites
################################################################################
...
# Input: meta-analysis DPS results
...
output_dir <- file.path(meta_dps_dir, "phosphoprotein_meta_gene_level_GSEA")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

# On Windows, convert to 8.3 short path name to avoid MAX_PATH (260 char) limit
# in deeply nested output subdirectories
if (.Platform$OS.type == "windows") {
    output_dir <- shortPathName(output_dir)
    cat("  Using short path:", output_dir, "\n")
}
...
# Options: "both" (default), "max_abs_z", or "median_z"
methods_to_run <- "both"
aggregation_methods <- if (methods_to_run == "both") {
    c("max_abs_z", "median_z")
} else {
    methods_to_run
}

# fgsea parameters (same as per-cancer gene-level GSEA pipeline)
minSize <- 15
maxSize <- 500

cat("====================================================================\n")
cat("TP53 Phosphoprotein Meta-Analysis Gene-Level Preranked GSEA\n")
cat("(without protein normalization)\n")
cat("====================================================================\n\n")
...
#' Aggregate phosphosite-level meta-analysis results to gene-level scores
#'
#' Reads a META_DPS CSV file and converts phosphosite-level Z_meta scores
#' to gene-level scores using one of two aggregation methods.
#'
#' @param meta_file Path to META_DPS CSV file (from Stouffer's meta-analysis)
#' @param method Aggregation method: "max_abs_z" or "median_z"
...
aggregate_meta_phospho_to_gene <- function(meta_file, method = "max_abs_z") {
    ...
    meta$ensembl_gene <- sub("\\|.*$", "", meta$phosphosite)

    # Strip Ensembl version suffix (e.g., ENSG00000183765.22 -> ENSG00000183765)
    meta$ensembl_gene <- sub("\\.[0-9]+$", "", meta$ensembl_gene)

    # Remove rows with missing Z_meta or gene IDs
    meta <- meta[is.finite(meta$Z_meta) & !is.na(meta$ensembl_gene) & meta$ensembl_gene != "", ]
    ...
    if (method == "max_abs_z") {
        # For each gene, select the phosphosite with the largest |Z_meta|,
        # then use its original (signed) Z_meta as the gene score
        gene_scores <- meta %>%
            group_by(ensembl_gene) %>%
            slice_max(abs(Z_meta), n = 1, with_ties = FALSE) %>%
            ungroup()
        stats_vec <- setNames(gene_scores$Z_meta, gene_scores$ensembl_gene)
    } else if (method == "median_z") {
        # For each gene, compute the median Z_meta across all phosphosites
        gene_scores <- meta %>%
            group_by(ensembl_gene) %>%
            summarise(z_median = median(Z_meta, na.rm = TRUE), .groups = "drop")
        stats_vec <- setNames(gene_scores$z_median, gene_scores$ensembl_gene)
    } else {
...
# ==============================================================================
# Section 5: Main Processing Loop (per aggregation method, per comparison)
# ==============================================================================
...
for (agg_method in aggregation_methods) {
    ...
    if (agg_method == "max_abs_z") {
        cat("  (gene score = Z_meta of phosphosite with largest |Z_meta| per gene)\n")
    } else {
        cat("  (gene score = median Z_meta across all phosphosites per gene)\n")
    }
    cat("####################################################################\n\n")
    ...
    for (coll_name in names(collections)) {
        ...
        coll_out <- file.path(method_output_dir, coll_name)
        dir.create(coll_out, recursive = TRUE, showWarnings = FALSE)

        # Summary row for this collection (one row summarizing all comparisons)
        summary_row <- list(collection = coll_name)

        for (j in seq_len(nrow(comparisons))) {
            comp_name <- comparisons$name[j]
            comp_label <- comparisons$label[j]

            meta_file <- file.path(meta_dps_dir, paste0("META_DPS_", comp_name, ".csv"))
            cat("    ", comp_label, ":", sep = "")

            # Aggregate phosphosites to gene-level scores
            stats_vec <- aggregate_meta_phospho_to_gene(meta_file, method = agg_method)
            ...
            if (!is.null(res)) {
                ...
                fwrite(res, file.path(coll_out, paste0("META_GSEA_gene_", comp_name, ".csv")))
                ...
                saveWorkbook(wb, file.path(coll_out, paste0("META_GSEA_gene_", comp_name, ".xlsx")),
                             overwrite = TRUE)
    ...
    for (coll_name in names(all_summaries)) {
        ...
        fwrite(summary_df, file.path(method_output_dir,
            paste0("META_GSEA_gene_summary_", coll_name, ".csv")))
        ...
        saveWorkbook(wb_sum, file.path(method_output_dir,
            paste0("META_GSEA_gene_summary_", coll_name, ".xlsx")), overwrite = TRUE)

        # Print significant counts only
        sig_cols <- grep("_sig$", colnames(summary_df), value = TRUE)
        if (length(sig_cols) > 0) {
            print_df <- summary_df[, c("collection", sig_cols), drop = FALSE]
...
cat("  max_abs_z/ : Gene score = Z_meta of strongest phosphosite per gene\n")
cat("  median_z/  : Gene score = median Z_meta across all phosphosites\n")
cat("====================================================================\n")
