################################################################################
# GSEA (PTMsigDB) Phosphosite Polar Heatmap (Circular Heatmap) - Without Protein Normalization
#
# Purpose:
#   Visualize phosphosite-level differential expression results (logFC) for
#   phosphosites of specified PTMsigDB pathways across multiple CPTAC datasets
#   using a circular (polar) heatmap. Each dataset is a concentric ring.
#   Phosphosites are arranged around the circle. Color encodes logFC, and cells
#   with adj.P.Val < 0.05 are marked with a black "*".
#
# Input:
#   - phosphoprotein_differential_analysis_without_protein_normalization_subgroup_adjusted/
#     {dataset}/DPS_{comparison}.csv
#   - PTMsigDB (data_PTMsigDB_all_sites_v2.0.0.xlsx)
#
# Output:
#   - GSEA_without_PN_phosphosites_heatmap/{color_scheme}/{comparison}/{pathway_name}.tiff
#   - GSEA_without_PN_phosphosites_heatmap/{color_scheme}/{comparison}/{pathway_name}.pdf
#   - GSEA_without_PN_phosphosites_heatmap/data/{comparison}/{pathway_name}.csv
#   - GSEA_without_PN_phosphosites_heatmap/data/{comparison}/{pathway_name}.xlsx
################################################################################
...
# Input directory (DPS results from phosphoprotein_differential_analysis_without_protein_normalization)
input_dir <- file.path(base_path, "phosphoprotein_differential_analysis_without_protein_normalization_subgroup_adjusted")

# Output directory
output_dir <- file.path(base_path, "GSEA_without_PN_phosphosites_heatmap")
...
selected_datasets <- c("COAD", "GBM", "OV", "PDAC", "UCEC")
...
# USER: Enable or disable hierarchical clustering for phosphosite ordering
#       (around the circle) and dataset ordering (concentric ring order,
#       outer to inner).
#       When disabled, axes fall back to alphabetical order.

cluster_phosphosites <- TRUE   # Cluster phosphosites around the circle?
...
# ==============================================================================
# Section 2: Define PTMsigDB Pathways to Plot
# ==============================================================================

# USER: Modify this list to add/remove PTMsigDB pathways for heatmaps.
# Each entry is a PTMsigDB signature name (e.g., "KINASE-PSP_CDK1").
#
# Examples of PTMsigDB pathway names:
#   "KINASE-PSP_CDK1", "KINASE-PSP_CDK2", "PERT-PSP_NOCODAZOLE",
#   "KINASE-iKiP_CSNK2A2.CK2A2", "KINASE-PSP_CK2A1/CSNK2A1",
#   "KINASE-PSP_ATM", "KINASE-PSP_AurB/AURKB", "PATH-BI_ISCHEMIA",
#   "KINASE-iKiP_ACVR1.ALK2", "KINASE-iKiP_PLK3"
...
cat("====================================================================\n")
cat("GSEA (PTMsigDB) Phosphosite Polar Heatmap (Without Protein Normalization)\n")
cat("====================================================================\n\n")

# ==============================================================================
# Section 4: Retrieve Pathway-Phosphosite Mappings from PTMsigDB
# ==============================================================================

cat("Loading pathway-phosphosite mappings from PTMsigDB ...\n")

# Load PTMsigDB from xlsx file
ptmsigdb_file <- file.path(base_path, "data_PTMsigDB_all_sites_v2.0.0.xlsx")
if (!file.exists(ptmsigdb_file)) {
    stop("PTMsigDB file not found: ", ptmsigdb_file)
}

ptmsigdb_df <- readxl::read_xlsx(ptmsigdb_file)

# Validate required columns
req_cols <- c("signature", "site.annotation")
if (!all(req_cols %in% colnames(ptmsigdb_df))) {
    stop("PTMsigDB xlsx is missing required columns: ",
         paste(setdiff(req_cols, colnames(ptmsigdb_df)), collapse = ", "))
}

# Extract gene_site from site.annotation (e.g., "PPP1R12A_T696:15226371" -> "PPP1R12A_T696")
ptmsigdb_df$gene_site <- toupper(sub(":.*$", "", trimws(ptmsigdb_df$site.annotation)))

# Build pathway lists: pathway_name -> character vector of gene_site identifiers
# These gene_site identifiers (e.g., "TP53_S315") match the "gene_symbol_phosphosite"
# column in the DPS files.
all_ptmsigdb_pathways <- split(ptmsigdb_df$gene_site, ptmsigdb_df$signature)
all_ptmsigdb_pathways <- lapply(all_ptmsigdb_pathways, function(v) unique(v[nzchar(v)]))
all_ptmsigdb_pathways[lengths(all_ptmsigdb_pathways) == 0] <- NULL

cat("  PTMsigDB loaded:", length(all_ptmsigdb_pathways), "total pathways\n")

# Filter to user-selected pathways
pathway_phosphosites <- list()  # pathway_name -> character vector of gene_symbol_phosphosite

for (pw in pathways_to_plot) {
    if (pw %in% names(all_ptmsigdb_pathways)) {
        sites <- sort(unique(all_ptmsigdb_pathways[[pw]]))
        pathway_phosphosites[[pw]] <- sites
        cat("  Pathway:", pw, "->", length(sites), "phosphosites\n")
    } else {
        cat("  [WARNING] Pathway not found in PTMsigDB:", pw, "\n")
    }
}

if (length(pathway_phosphosites) == 0) {
    stop("No valid pathways found. Please check pathways_to_plot definitions.")
}
...
#'
#' @param logfc_mat Numeric matrix (phosphosites x datasets) of logFC values
#' @param sig_mat   Logical matrix (phosphosites x datasets) TRUE = adj.P.Val < 0.05
...
#' @param title_sub  Subtitle (pathway name)
#' @param site_dend  Dendrogram object for phosphosites (NULL if no clustering)
...
draw_polar_heatmap <- function(logfc_mat, sig_mat, col_fun, na_col = "grey90",
                                ...
                                site_dend = NULL, show_dendro = TRUE,
                                show_stars = FALSE,
                                gap_deg = 55, track_w = "auto") {

    n_sites    <- nrow(logfc_mat)
    n_datasets <- ncol(logfc_mat)

    # --- Adaptive sizing parameters ---
    site_label_cex <- if (n_sites > 100) 0.50
                      else if (n_sites > 60) 0.65
                      else if (n_sites > 30) 0.80
                      else 1.00

    sig_cex <- if (n_sites > 100) 0.35
               else if (n_sites > 60) 0.45
               else 0.55
    ...
    # Phosphosite label track height (needs space for rotated text)
    label_track_h <- if (n_sites > 100) 0.12
                     else if (n_sites > 60) 0.15
                     else 0.20
    ...
    circos.initialize(sectors = "heatmap", xlim = c(0.5, n_sites + 0.5))

    # --- Track 1 (outermost): Phosphosite labels ---
    circos.track(
        sectors      = "heatmap",
        ylim         = c(0, 1),
    ...
    if (show_dendro && !is.null(site_dend) && n_sites >= 2) {
        dend_height <- attr(site_dend, "height")
        circos.track(
            sectors      = "heatmap",
            ...
            bg.border    = NA,
            panel.fun    = function(x, y) {
                circos.dendrogram(site_dend, facing = "outside",
...
# ==============================================================================
# Section 6: Load DPS Data and Generate Polar Heatmaps
# ==============================================================================
...
for (j_comp in seq_len(nrow(comparisons))) {
    ...
    # --- Load DPS data from all selected datasets for this comparison ---
    dps_all <- list()

    for (ds in selected_datasets) {
        dps_file <- file.path(input_dir, ds, paste0("DPS_", comp_name, ".csv"))
        if (!file.exists(dps_file)) {
            cat("  [SKIP] DPS file not found for", ds, ":", basename(dps_file), "\n")
            next
        }
        ...
        required_cols <- c("gene_symbol_phosphosite", "logFC", "adj.P.Val")
    ...
    # --- Generate heatmap for each PTMsigDB pathway ---
    for (pathway_name in names(pathway_phosphosites)) {
        pathway_sites <- pathway_phosphosites[[pathway_name]]

        cat("\n  Pathway:", pathway_name, "(", length(pathway_sites), "phosphosites )\n")

        # Filter DPS data to phosphosites that belong to this PTMsigDB pathway
        plot_data <- dps_combined[gene_symbol_phosphosite %in% pathway_sites]
        ...
        plot_data <- plot_data[, .(gene_symbol_phosphosite, dataset, logFC, adj.P.Val)]

        # Handle duplicate gene_symbol_phosphosite per dataset (keep first occurrence)
        plot_data <- plot_data[!duplicated(plot_data[, .(gene_symbol_phosphosite, dataset)])]

        # Report coverage
        sites_found <- sort(unique(plot_data$gene_symbol_phosphosite))
        datasets_found <- sort(unique(plot_data$dataset))
        cat("    Phosphosites found:", length(sites_found), "/", length(pathway_sites), "\n")
        cat("    Datasets found:", length(datasets_found), "\n")

        # Create wide matrices for logFC and adj.P.Val
        logfc_wide <- dcast(plot_data, gene_symbol_phosphosite ~ dataset, value.var = "logFC")
        pval_wide  <- dcast(plot_data, gene_symbol_phosphosite ~ dataset, value.var = "adj.P.Val")
        ...
        rownames(pval_mat) <- pval_mat$gene_symbol_phosphosite
        pval_mat$gene_symbol_phosphosite <- NULL
        pval_mat <- as.matrix(pval_mat)

        # Significance matrix (TRUE if adj.P.Val < 0.05)
        sig_mat <- !is.na(pval_mat) & pval_mat < 0.05

        # Impute NA with 0 for distance calculation (NA = phosphosite not measured)
        logfc_mat_imputed <- logfc_mat
        logfc_mat_imputed[is.na(logfc_mat_imputed)] <- 0

        # --- Phosphosite clustering (determines order around the circle) ---
        site_dend <- NULL
        if (cluster_phosphosites && nrow(logfc_mat_imputed) >= 2) {
            site_dist   <- dist(logfc_mat_imputed, method = cluster_distance)
            site_hclust <- hclust(site_dist, method = cluster_method)
            site_order  <- rownames(logfc_mat_imputed)[site_hclust$order]
            site_dend   <- as.dendrogram(site_hclust)
            cat("    Phosphosite clustering: distance =", cluster_distance,
                ", method =", cluster_method, "\n")
        } else {
            site_order <- sort(rownames(logfc_mat))
            if (cluster_phosphosites && nrow(logfc_mat_imputed) < 2) {
                cat("    [NOTE] Too few phosphosites for clustering, using alphabetical order\n")
            }
        }
        ...
        logfc_mat_plot <- logfc_mat[site_order, ds_order, drop = FALSE]
        sig_mat_plot   <- sig_mat[site_order, ds_order, drop = FALSE]
        ...
        # Sanitize pathway name for filenames (replace / and other special chars)
        pathway_filename <- gsub("[/\\\\:*?\"<>|]", "_", pathway_name)

        # Prepare export data: wide format with logFC and significance
        export_data <- merge(logfc_wide, pval_wide,
                              by = "gene_symbol_phosphosite", suffixes = c("_logFC", "_adjPVal"))
        export_data <- export_data[order(export_data$gene_symbol_phosphosite), ]

        # Save CSV
        data_csv <- file.path(comp_data_dir, paste0(pathway_filename, ".csv"))
        fwrite(export_data, data_csv)

        # Save XLSX
        data_xlsx <- file.path(comp_data_dir, paste0(pathway_filename, ".xlsx"))
        ...
        for (cs_name in selected_color_schemes) {
            ...
            n_sites_plot    <- nrow(logfc_mat_plot)
            n_datasets_plot <- ncol(logfc_mat_plot)
            plot_dim <- max(8, 6 + n_sites_plot * 0.015 + n_datasets_plot * 0.1)
            ...
            # --- Save TIFF ---
            tiff_file <- file.path(comp_cs_dir, paste0(pathway_filename, ".tiff"))
            ...
            draw_polar_heatmap(
                ...
                title_sub  = paste0("Pathway: ", pathway_name, " (", cs$label, ")"),
                site_dend  = site_dend,
                show_dendro = cluster_phosphosites,
            ...
            # --- Save PDF ---
            pdf_file <- file.path(comp_cs_dir, paste0(pathway_filename, ".pdf"))
...
cat("====================================================================\n")
cat("GSEA (PTMsigDB) phosphosite polar heatmaps completed!\n")
