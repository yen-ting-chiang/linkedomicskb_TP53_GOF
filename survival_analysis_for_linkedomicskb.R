...
output_dir <- file.path(base_path, "survival_analysis")
...
for (cancer in cancer_types) {
  ...
  for (comp_name in names(comparisons)) {
    ...
    get_valid_covariates <- function(data) {
      ...
      if ("Sex" %in% colnames(data) && length(unique(na.omit(data$Sex))) > 1) valid_cov <- c(valid_cov, "Sex")
      return(valid_cov)
    }
