...
# (Meta-analysis KSEA leading edges x per-dataset DPS with PN logFC)
#
# Purpose:
#   For each selected KSEA kinase, extract its leading edge substrates from the
#   meta-KSEA results (with protein normalization), then look up the logFC of
#   each substrate's corresponding gene_symbol_phosphosite in the per-dataset
#   DPS results (with protein normalization). The resulting heatmap shows:
...
#
# Output:
#   phosphoprotein_DA_with_PN_meta_analysis_KSEA_leadingEdge_heatmap/
...
# Meta-KSEA input directory (with protein normalization meta-analysis results)
meta_ksea_dir <- file.path(base_path,
                           "phosphoprotein_differential_analysis_subgroup_adjusted",
                           "phosphoprotein_DPS_meta_analysis",
                           "meta_KSEA")

# Per-dataset DPS input directory (with protein normalization)
...
cat("Meta-KSEA leading edges x per-dataset DPS with PN logFC\n")
cat("====================================================================\n\n")
...
for (j_comp in seq_len(nrow(comparisons))) {
  ...
  # ------------------------------------------------------------------
  # Step 2: Load per-dataset DPS data (with PN) for this comparison
  # ------------------------------------------------------------------
