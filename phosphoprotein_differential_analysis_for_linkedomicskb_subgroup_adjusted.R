...
output_dir <- file.path(base_path, "phosphoprotein_differential_analysis_subgroup_adjusted")
...
load_phosphosite_matrix <- function(ds_id, ds_dir, protein_adjust = TRUE) {
...
run_limma_group_comparison <- function(M, group_vec, purity_vec, sa_df) {
    ...
    if (!is.null(sa_df)) {
        ...
        for (cn in setdiff(colnames(sa_df), "Sample")) {
            v <- sa_df[[cn]][m_idx]
            if (sum(!is.na(v)) > 0) {
                if (is.character(v) || is.factor(v)) {
                    v_fac <- factor(v)
                    lv <- levels(droplevels(v_fac[!is.na(v_fac)]))
                    if (length(lv) > 1) {
                        DF[[cn]] <- v_fac
                    } else {
    ...
    res_df <- nrow(des) - rnk
    if (res_df <= 0) {
        drop_order <- intersect(c(setdiff(colnames(DF), c("group", "Sex", "Age", "purity")), "Sex", "Age", "purity"), colnames(DF))
...
for (i in seq_len(nrow(datasets))) {
    ...
    mat0 <- tryCatch(
        load_phosphosite_matrix(ds_folder, ds_dir, protein_adjust = TRUE),
        error = function(e) {
    ...
    if (!is.null(purity_vec)) {
        p <- suppressWarnings(as.numeric(purity_vec$Purity))
    ...
    extra_cov_names <- setdiff(colnames(sa_df), c("Sample", "Age", "Sex"))
