################################################################################
# Gene Set Protein Heatmap (pheatmap)
#
# Purpose:
...
#   Dendrograms are drawn on clustered axes via pheatmap.
#
# Input:
...
#       and the corresponding dendrogram is hidden.
...
#   "correlation" : 1 - Pearson correlation (captures pattern similarity)
...
# ==============================================================================
# Section 3: Define RColorBrewer Diverging Color Palettes
# ==============================================================================
# Diverging color schemes from the RColorBrewer package, commonly used in pheatmap.
# You can choose one or more of these palettes to generate heatmaps.

color_schemes <- list(
  # 1. Red-Yellow-Blue (RdYlBu) - pheatmap default
  "RdYlBu" = list(
    name   = "RdYlBu",
    label  = "Red-Yellow-Blue (pheatmap default)"
  ),
  # 2. Red-Blue (RdBu) - High-impact journal favorite
  "RdBu" = list(
    name   = "RdBu",
    label  = "Red-Blue (Nature/Cell style)"
  ),
  # 3. Purple-Orange (PuOr) - Colorblind friendly
  "PuOr" = list(
    name   = "PuOr",
    label  = "Purple-Orange"
  ),
  ...
  # 6. Brown-BlueGreen (BrBG)
  "BrBG" = list(
    name   = "BrBG",
    label  = "Brown-BlueGreen"
  ),
  # 7. Spectral (Spectral)
  "Spectral" = list(
    name   = "Spectral",
...
# Select one or more from: "RdYlBu", "RdBu", "PuOr", "PRGn", "PiYG", "BrBG", "Spectral"
# To generate all schemes, keep it as: names(color_schemes)

selected_color_schemes <- c("RdYlBu", "RdBu", "PuOr")

cat("====================================================================\n")
cat("Gene Set Protein Heatmap (pheatmap)\n")
cat("====================================================================\n\n")
...
# ==============================================================================
# Section 5: Note on Color Palette Generation
# ==============================================================================
# The previous manual color ramp helper function has been deprecated, as we now
# leverage the RColorBrewer package directly within the main loop to dynamically
# generate smooth, publications-grade diverging palettes for pheatmap.

# ==============================================================================
# Section 6: Load DEG Data and Generate Heatmaps
# ==============================================================================
...
# Build custom distance function for pheatmap (to support "correlation")
make_dist_func <- function(method) {
  function(x) {
    if (method == "correlation") {
      # 1 - Pearson correlation as distance
      as.dist(1 - cor(t(as.matrix(x)), use = "pairwise.complete.obs"))
    } else {
...
for (j in seq_len(nrow(comparisons))) {
  ...
  for (gs_name in names(gene_set_genes)) {
    ...
    # Convert to numeric matrices with gene symbols as rownames
    ...
    # Build significance marker matrix ("*" if adj.P.Val < 0.05, "" otherwise)
    ...
    # For pheatmap: impute NA with 0 in the matrix used for distance calc
    logfc_mat_for_clust <- logfc_mat
    logfc_mat_for_clust[is.na(logfc_mat_for_clust)] <- 0
    
    # Row clustering (genes)
    if (cluster_genes && nrow(logfc_mat) >= 2) {
      row_dist <- make_dist_func(cluster_distance)(logfc_mat_for_clust)
      row_hclust <- hclust(row_dist, method = cluster_method)
      do_cluster_rows <- row_hclust
      cat("    Gene clustering: distance =", cluster_distance,
          ", method =", cluster_method, "\n")
    } else {
      do_cluster_rows <- FALSE
      if (cluster_genes && nrow(logfc_mat) < 2) {
        cat("    [NOTE] Too few genes for clustering, using alphabetical order\n")
      }
    }
    
    # Column clustering (datasets)
    if (cluster_datasets && ncol(logfc_mat) >= 2) {
      col_dist <- make_dist_func(cluster_distance)(t(logfc_mat_for_clust))
      col_hclust <- hclust(col_dist, method = cluster_method)
      do_cluster_cols <- col_hclust
    ...
    plot_width  <- max(4.5, 1.5 + n_datasets * 0.55 + 1.8)
    plot_height <- max(4.0, 1.2 + n_genes * 0.18 + 1.2)
    
    # --- Generate heatmaps for each selected color scheme ---
    for (cs_name in selected_color_schemes) {
      # Dynamically fetch the diverging palette from RColorBrewer.
      # RColorBrewer's diverging palettes (e.g. RdBu, RdYlBu) support up to 11 classes.
      raw_colors <- RColorBrewer::brewer.pal(n = 11, name = cs_name)
      
      # Reverse the color vector so that negative logFC values correspond to cool colors
      # (e.g., blue) and positive logFC values correspond to warm colors (e.g., red).
      heatmap_colors <- colorRampPalette(rev(raw_colors))(101)
      
      # Define symmetric breaks centered at 0
      breaks <- seq(-logfc_lim, logfc_lim, length.out = length(heatmap_colors) + 1)
      ...
      pheatmap_args <- list(
        ...
        treeheight_row       = ifelse(isFALSE(do_cluster_rows), 0, 40),
        treeheight_col       = ifelse(isFALSE(do_cluster_cols), 0, 40),
      ...
      p_obj <- do.call(pheatmap, pheatmap_args)
      grid::grid.newpage()
