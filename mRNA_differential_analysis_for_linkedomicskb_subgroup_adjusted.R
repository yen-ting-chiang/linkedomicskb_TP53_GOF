...
# Methodology adapted from protein_differential_analysis pipeline for mRNA data
################################################################################
...
# Output directory
output_dir <- file.path(base_path, "mRNA_differential_analysis_subgroup_adjusted")
...
# ==============================================================================
# Section 2: Utility Functions (adapted for mRNA data)
# ==============================================================================

#' Load mRNA quantification matrix (RNAseq RSEM, log2 UQ-1500 normalized)
load_mRNA_matrix <- function(ds_dir, ds_cancer) {
    fp <- file.path(ds_dir, paste0(ds_cancer, "_RNAseq_gene_RSEM_coding_UQ_1500_log2_Tumor.txt"))
    ...
    # Identify gene column (ENSEMBL IDs with version, e.g. ENSG00000000003.15)
    gcol <- names(dat)[1]
...
get_cancer_specific_covariates <- function(ds_dir, ds_cancer, sample_ids) {
    res <- data.frame(row.names = sample_ids)
...
run_limma_group_comparison <- function(mat, group_vec,
    ...
    # Add sex, age, and any cancer-specific covariates
    if (!is.null(sa_df)) {
        for (cn in colnames(sa_df)) {
            v <- sa_df[so, cn]
            if (sum(!is.na(v)) > 0) {
                DF[[cn]] <- v
            }
    ...
    group_cols <- make.names(paste0("group", lvls))
...
for (i in seq_len(nrow(datasets))) {
    ...
    # --- Load mRNA matrix (already log2 UQ-1500 normalized) ---
    mat0 <- load_mRNA_matrix(ds_dir, ds_cancer)
    ...
    extra_cov <- get_cancer_specific_covariates(ds_dir, ds_cancer, colnames(mat))
    if (ncol(extra_cov) > 0) {
        sa_df <- cbind(sa_df, extra_cov)
    }
    ...
    extra_cov_names <- setdiff(colnames(sa_df), c("age", "sex"))
    for (ec in extra_cov_names) {
        v <- sa_df[[ec]]
        miss_cnt <- sum(is.na(v))
        imp_val <- NA
        if (sum(!is.na(v)) >= n_samples * 0.8) {
            imp_val <- "Unknown"
            if (miss_cnt > 0) {
                v[is.na(v)] <- imp_val
                sa_df[[ec]] <- v
...
if (length(imputation_report_all) > 0) {
    imp_df <- dplyr::bind_rows(imputation_report_all)
