################################################################################
# TP53 Phosphoprotein GSEA (without Protein Normalization)
# Cross-Dataset Meta-Analysis
#
# Method: Stouffer's Weighted Z-score Method (pathway-level)
#   1. Read per-cancer GSEA results (NES, pval) from
#      phosphoprotein_GSEA_without_protein_normalization_subgroup_adjusted/
#      {cancer_type}/{collection}/
...
#
# Input:  phosphoprotein_GSEA_without_protein_normalization_subgroup_adjusted/
# Output: phosphoprotein_GSEA_without_PN_meta/
################################################################################
...
gsea_input_dir <- file.path(base_path, "phosphoprotein_GSEA_without_protein_normalization_subgroup_adjusted")

output_dir <- file.path(base_path, "phosphoprotein_GSEA_without_PN_meta")
...
cat("====================================================================\n")
cat("TP53 Phosphoprotein GSEA (without Protein Normalization)\n")
...
# ==============================================================================
# Section 4: Main Meta-Analysis Loop (per collection x per comparison)
# ==============================================================================
...
for (coll_name in collection_names) {
    ...
    for (j in seq_len(nrow(comparisons))) {
        ...
        # --- Step 5b: Compute per-pathway up/down dataset counts and names ---
        # A dataset is "significantly up" if its per-cancer pval < 0.05 and NES > 0
        # A dataset is "significantly down" if its per-cancer pval < 0.05 and NES < 0
        ...
        for (g in seq_len(nrow(meta_results))) {
            pw <- meta_results$pathway[g]
            pw_idx <- match(pw, all_pathways)
            if (is.na(pw_idx)) next

            pvals_row <- per_cancer_pval[pw_idx, ]
            nes_row <- per_cancer_NES[pw_idx, ]

            sig_up <- included_cancers[!is.na(pvals_row) & !is.na(nes_row) &
                                       pvals_row < 0.05 & nes_row > 0]
            sig_down <- included_cancers[!is.na(pvals_row) & !is.na(nes_row) &
                                         pvals_row < 0.05 & nes_row < 0]
        ...
        # First 7 columns: pathway, Z_meta, padj, up_datasets_num,
        #   down_datasets_num, up_datasets_name, down_datasets_name
        priority_cols <- c("pathway", "Z_meta", "padj",
...
cat("\n====================================================================\n")
cat("Phosphoprotein GSEA (without Protein Normalization)\n")
