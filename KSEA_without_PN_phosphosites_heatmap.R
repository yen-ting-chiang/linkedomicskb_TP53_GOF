################################################################################
# KSEA Phosphosite Polar Heatmap (Circular Heatmap) - Without Protein Normalization
#
# Purpose:
#   Visualize phosphosite-level differential expression results (logFC) for
#   substrates of specified KSEA kinases across multiple CPTAC datasets using a
#   circular (polar) heatmap. Each dataset is a concentric ring. Phosphosites
#   (substrates) are arranged around the circle. Color encodes logFC, and cells
...
#   - KSEAapp built-in KSData (kinase-substrate database from PhosphoSitePlus)
#
# Output:
#   - KSEA_without_PN_phosphosites_heatmap/{color_scheme}/{comparison}/{kinase_name}.tiff
#   - KSEA_without_PN_phosphosites_heatmap/{color_scheme}/{comparison}/{kinase_name}.pdf
#   - KSEA_without_PN_phosphosites_heatmap/data/{comparison}/{kinase_name}.csv
#   - KSEA_without_PN_phosphosites_heatmap/data/{comparison}/{kinase_name}.xlsx
################################################################################
...
# Output directory
output_dir <- file.path(base_path, "KSEA_without_PN_phosphosites_heatmap")
...
#selected_datasets <- c("GBM", "HNSCC", "LUAD", "UCEC")
...
# ==============================================================================
# Section 2: Define KSEA Kinases (Pathways) to Plot
# ==============================================================================

# USER: Modify this list to add/remove KSEA kinases (pathways) for heatmaps.
# Each entry is a kinase gene name from the KSEAapp KSData database.
# Note: KSEA does NOT have collections (unlike MSigDB), so simply list kinase names.
#
# Examples of commonly studied kinases:
#   "CDK1", "CDK2", "MELK", "CSNK2A1", "AURKA", "AKT1",
#   "MAPK1", "MAPK3", "CHEK1", "CHEK2", "ATM", "ATR",
#   "PLK1", "CDK4", "CDK6", "MTOR", "RPS6KB1", "GSK3B"
...
cat("====================================================================\n")
cat("KSEA Phosphosite Polar Heatmap (Without Protein Normalization)\n")
cat("====================================================================\n\n")

# ==============================================================================
# Section 4: Retrieve Kinase-Substrate Mappings from KSEAapp KSData
# ==============================================================================

cat("Loading kinase-substrate mappings from KSEAapp KSData ...\n")

# Load KSData from KSEAapp
data("KSData", package = "KSEAapp", envir = environment())

# KSData is a data.frame with columns including:
#   GENE        : Kinase gene symbol (e.g., "CDK1", "AKT1")
#   SUB_GENE    : Substrate gene symbol (e.g., "TP53", "RB1")
#   SUB_MOD_RSD : Substrate modification residue (e.g., "S315", "T826")
# We construct substrate identifiers in "Gene_Residue" format (e.g., "TP53_S315")
# to match the "gene_symbol_phosphosite" column in the DPS files.

kinase_substrates <- list()  # kinase_name -> character vector of gene_symbol_phosphosite

for (kinase in kinases_to_plot) {
    ks_df <- KSData[KSData$GENE == kinase, , drop = FALSE]

    if (nrow(ks_df) == 0) {
        cat("  [WARNING] Kinase not found in KSData:", kinase, "\n")
        next
    }

    # Build gene_symbol_phosphosite identifiers: Gene_Residue (e.g., "TP53_S315")
    substrates <- paste0(ks_df$SUB_GENE, "_", ks_df$SUB_MOD_RSD)
    substrates <- unique(sort(substrates))

    kinase_substrates[[kinase]] <- substrates
    cat("  Kinase:", kinase, "->", length(substrates), "substrates\n")
}

if (length(kinase_substrates) == 0) {
    stop("No valid kinases found. Please check kinases_to_plot definitions.")
}
...
#' @param title_sub  Subtitle (kinase name)
...
for (j_comp in seq_len(nrow(comparisons))) {
    ...
    # --- Generate heatmap for each kinase ---
    for (kinase_name in names(kinase_substrates)) {
        kinase_subs <- kinase_substrates[[kinase_name]]

        cat("\n  Kinase:", kinase_name, "(", length(kinase_subs), "substrates )\n")

        # Filter DPS data to phosphosites that are substrates of this kinase
        plot_data <- dps_combined[gene_symbol_phosphosite %in% kinase_subs]
        ...
        cat("    Phosphosites found:", length(sites_found), "/", length(kinase_subs), "\n")
        ...
        # Save CSV
        data_csv <- file.path(comp_data_dir, paste0(kinase_name, ".csv"))
        fwrite(export_data, data_csv)

        # Save XLSX
        data_xlsx <- file.path(comp_data_dir, paste0(kinase_name, ".xlsx"))
        ...
        for (cs_name in selected_color_schemes) {
            ...
            # --- Save TIFF ---
            tiff_file <- file.path(comp_cs_dir, paste0(kinase_name, ".tiff"))
            ...
            draw_polar_heatmap(
                ...
                title_sub  = paste0("Kinase: ", kinase_name, " (", cs$label, ")"),
                site_dend  = site_dend,
            ...
            # --- Save PDF ---
            pdf_file <- file.path(comp_cs_dir, paste0(kinase_name, ".pdf"))
...
cat("====================================================================\n")
cat("KSEA phosphosite polar heatmaps completed!\n")
